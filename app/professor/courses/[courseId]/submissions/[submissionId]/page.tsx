...
export default async function SubmissionDetailPage({params}:{params:Promise<{courseId:string;submissionId:string}>}){
  ...
  if(!s) return <AppShell userName={profile?.full_name||profile?.email}><div className="alert">SubmissÃ£o nÃ£o encontrada.</div></AppShell>;
  const submission:any=s; const score=submission.scores?.[0]; const fb=submission.feedback?.[0];
  return <AppShell userName={profile?.full_name||profile?.email}>
    <section className="hero"><div><div className="eyebrow">{submission.assignment?.patient?.code} Â· Semana {submission.episode?.week_no}</div><h1>{submission.assignment?.patient?.display_name}</h1><p>{submission.episode?.title}</p></div><div className="row"><StatusBadge value={s.status}/><Link className="btn btn-secondary" href={`/professor/courses/${courseId}/submissions`}>Voltar</Link></div></section>
    <section className="card" style={{marginBottom:24}}><h2>SÃ­ntese farmacÃªutica</h2><p style={{whiteSpace:"pre-wrap"}}>{s.summary}</p></section>
    <section className="stack" style={{marginBottom:24}}><h2 style={{margin:0}}>Problemas, intervenÃ§Ãµes e monitoramento</h2>{[...(s.problems||[])].sort((a:any,b:any)=>a.sort_order-b.sort_order).map((p:any,i:number)=><div className="card" key={p.id}><div className="row-between"><h3 style={{margin:0}}>Problema {i+1}</h3><span className="badge">{p.category}</span></div><p><strong>{p.description}</strong></p><p className="muted">EvidÃªncia: {p.evidence}</p><div className="divider"/><h4>IntervenÃ§Ãµes</h4>{(p.interventions||[]).map((it:any)=><div className="card card-subtle" key={it.id}><div className="row-between"><strong>{it.recommendation}</strong><span className="badge">{it.priority}</span></div><div className="small muted">Alvo: {it.target||"â€”"}</div></div>)}<h4>Monitoramento</h4>{(p.monitoring||[]).map((m:any)=><div className="small" key={m.id}><strong>{m.parameter}</strong> Â· meta {m.target||"â€”"} Â· prazo {m.timeframe||"â€”"}</div>)}</div>)}</section>
    <section className="card"><h2>CorreÃ§Ã£o</h2><form action={gradeSubmission.bind(null,courseId,submissionId)} className="stack"><div className="field"><label>Nota total</label><input className="input" name="total_points" type="number" step="0.1" min="0" defaultValue={score?.total_points??""} required/></div><div className="field"><label>Feedback</label><textarea className="textarea" name="feedback" defaultValue={fb?.text||""} required/></div><label className="row"><input type="checkbox" name="release_now" defaultChecked={!!fb?.released_at}/> Liberar feedback ao aluno agora</label><button className="btn" type="submit">Salvar correÃ§Ã£o</button></form>{s.status!=="draft"&&s.status!=="reopened"?<><div className="divider"/><form action={reopenSubmission.bind(null,courseId,submissionId)}><button className="btn btn-secondary" type="submit">Reabrir para ediÃ§Ã£o</button></form></>:null}</section>
  </AppShell>
}
