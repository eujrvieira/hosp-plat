...
export default async function StudentEpisodePage({ params }: { params: Promise<{ assignmentId: string; episodeId: string }> }) {
  ...
  if(!assignment) return <AppShell userName={profile?.full_name||profile?.email}><div className="alert">Atribuição indisponível.</div></AppShell>;
  ...
  if(!episode) return <AppShell userName={profile?.full_name||profile?.email}><div className="alert">Este episódio ainda não está liberado para você.</div></AppShell>;
  ...
  return (
    <AppShell userName={profile?.full_name||profile?.email}>
      <section className="hero"><div><div className="eyebrow">Semana {episode.week_no} · {assignment.patient?.display_name}</div><h1>{episode.title}</h1><p>{episode.summary}</p></div><Link className="btn btn-secondary" href={`/student/assignments/${assignmentId}`}>Voltar ao caso</Link></section>
      ...
      <div className="grid grid-2" style={{marginBottom:24}}>
        <section className="card"><h2>Sinais vitais</h2>{(episode.vital_signs||[]).length?<div className="table-wrap"><table><thead><tr><th>Hora</th><th>FC</th><th>FR</th><th>PA</th><th>SpO₂</th><th>T</th></tr></thead><tbody>{(episode.vital_signs||[]).map((v:any)=><tr key={v.id}><td>{formatDateTime(v.measured_at)}</td><td>{v.heart_rate??"—"}</td><td>{v.respiratory_rate??"—"}</td><td>{v.systolic_bp??"—"}/{v.diastolic_bp??"—"}</td><td>{v.spo2??"—"}%</td><td>{v.temperature_c??"—"}°C</td></tr>)}</tbody></table></div>:<div className="empty">Sem registros nesta semana.</div>}</section>
        <section className="card"><h2>Exames laboratoriais</h2>{(episode.lab_results||[]).length?<div className="table-wrap"><table><thead><tr><th>Exame</th><th>Resultado</th><th>Referência</th></tr></thead><tbody>{(episode.lab_results||[]).map((l:any)=><tr key={l.id}><td>{l.test_name}</td><td><strong>{l.value_text}</strong> {l.unit}</td><td>{l.reference_range||"—"}</td></tr>)}</tbody></table></div>:<div className="empty">Sem exames nesta semana.</div>}</section>
      </div>
      <div className="grid grid-2" style={{marginBottom:24}}>
        <section className="card"><h2>Prescrição</h2>{rxItems.length?<div className="stack">{rxItems.map((i:any)=><div className="card card-subtle" key={i.id}><strong>{i.drug}</strong><div>{i.concentration} · {i.dose} · {i.route} · {i.frequency}</div><div className="small muted">{i.indication||"Sem indicação registrada"}</div></div>)}</div>:<div className="empty">Sem itens prescritos.</div>}</section>
        <section className="card"><h2>Evoluções e eventos</h2><div className="timeline">{[...(episode.clinical_notes||[]).map((n:any)=>({id:n.id,at:n.note_at,title:`${n.profession} · ${n.note_type}`,text:n.text})),...(episode.clinical_events||[]).map((e:any)=>({id:e.id,at:e.event_at,title:e.title,text:e.description}))].sort((a:any,b:any)=>new Date(a.at).getTime()-new Date(b.at).getTime()).map((n:any)=><div className="timeline-item" key={n.id}><div className="timeline-time">{formatDateTime(n.at)}</div><div className="timeline-content"><h4>{n.title}</h4><div>{n.text}</div></div></div>)}</div></section>
      </div>
      <section className="card"><div className="row-between"><h2>Avaliação farmacêutica</h2>{submission?<StatusBadge value={submission.status}/>:null}</div>
