...
export default async function StudentAssignmentPage({ params }: { params: Promise<{ assignmentId: string }> }) {
  ...
  if (!assignment) return <AppShell userName={profile?.full_name || profile?.email}><div className="alert">Você não possui acesso a este caso.</div></AppShell>;
  ...
  return (
    <AppShell userName={profile?.full_name || profile?.email}>
      <section className="hero"><div><div className="eyebrow">{patient.code} · {assignment.course?.name}</div><h1>{patient.display_name}</h1><p>{patient.baseline_summary}</p></div><StatusBadge value={assignment.state}/></section>
      <section className="card" style={{marginBottom:24}}><h2>Dados basais</h2><dl className="kv"><dt>Idade</dt><dd>{patient.age ?? "—"}</dd><dt>Sexo</dt><dd>{patient.sex ?? "—"}</dd><dt>Peso</dt><dd>{patient.weight_kg ? `${patient.weight_kg} kg` : "—"}</dd><dt>Altura</dt><dd>{patient.height_cm ? `${patient.height_cm} cm` : "—"}</dd><dt>Ramo atual</dt><dd>{assignment.current_branch_key}</dd></dl></section>
      <section className="stack"><div className="row-between"><h2 style={{margin:0}}>Linha do tempo liberada</h2><span className="small muted">Somente episódios liberados ficam acessíveis.</span></div>{(episodes||[]).length ? <div className="grid grid-2">{(episodes||[]).map((e:any)=>{ const sub=latestByEpisode.get(e.id); return <Link className="card" key={e.id} href={`/student/assignments/${assignmentId}/episodes/${e.id}`}><div className="row-between"><div><div className="eyebrow">Semana {e.week_no}</div><h3 style={{margin:"4px 0"}}>{e.title}</h3></div>{sub ? <StatusBadge value={sub.status}/> : <span className="badge">pendente</span>}</div><p>{e.summary}</p><div className="small muted">Liberado: {formatDateTime(e.release_at)} · Prazo: {formatDateTime(e.due_at)}</div></Link>})}</div> : <div className="empty">Nenhum episódio foi liberado ainda.</div>}</section>
    </AppShell>
  );
}
